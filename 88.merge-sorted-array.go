...
// Merge merges two sorted arrays nums1 and nums2 into a single array sorted in non-decreasing order
func merge(nums1 []int, m int, nums2 []int, n int) {
	// Initialize a counter for nums2
	cnt := 0
	// Loop through the elements of nums1 from m to the end
	for i := m; i < len(nums1); i++ {
		// Set the value from nums2 to nums1
		nums1[i] = nums2[cnt]
		// Increment the counter for nums2
		cnt++
	}
