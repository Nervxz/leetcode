...
min(event_date) 
// renames the result of the MIN(event_date) function to first_login for clarity in the output
as first_login  from activity 
// the result set by the player_id column each unique player_id, the query will calculate the minimum event_date
group by player_id